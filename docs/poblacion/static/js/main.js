...
// Ejecutar simulación demográfica localmente (RK4 + Analítico)
function runLocalSimulation() {
    const P0 = parseFloat(sliderP0.value);
    ...
    // Ecuación diferencial: dP/dt = r * P * (1.0 - P / K)
    const f = (t, P) => r * P * (1.0 - P / K);

    const pop_numerical = new Array(steps);
    pop_numerical[0] = P0;

    for (let i = 0; i < steps - 1; i++) {
        const t = time[i];
        const curr_P = pop_numerical[i];
        const h = dt;

        const k1 = f(t, curr_P);
        const k2 = f(t + h / 2, curr_P + (h * k1) / 2);
        const k3 = f(t + h / 2, curr_P + (h * k2) / 2);
        const k4 = f(t + h, curr_P + h * k3);

        const next_P = curr_P + (h / 6) * (k1 + 2 * k2 + 2 * k3 + k4);
        pop_numerical[i + 1] = Math.max(0.0, next_P);
    }

    // Solución analítica exacta: P(t) = (K * P0 * exp_rt) / (K + P0 * (exp_rt - 1))
    const pop_analytical = time.map(t => {
        const exp_rt = Math.exp(r * t);
        return (K * P0 * exp_rt) / (K + P0 * (exp_rt - 1.0));
    });

    const years = time.map(t => 1950 + t);

    simData = {
        time: time,
        years: years,
        pop_numerical: pop_numerical,
...
function updateVisualsAtStep(step) {
    ...
    // Población de 0.5M a 15M -> Radio de 15km a 95km (15,000m a 95,000m)
